...
$$
B\left(x_1-x_0\right)-A\left(y_1-y_0\right)=0 \text {. }
$$
又因为 $P_1\left(x_1, y_1\right)$ 是直线 $l$ 上的点, 所以
$$
A x_1+B y_1+C=0 \text {. }
$$
如果联立(4)和(5)求出 $x_1, y_1$, 再代人(3)中求 $d$, 将是非常烦㺳的 (请 读者自行尝试). 注意到为了求出(3), 我们只需求出 $x_1-x_0$ 与 $y_1-y_0$ 的平 方和即可, 而(4)中正好有这两个式子, 但(2)中没有. 为此, 在(5)的左右两边 同时椷去 $A x_0, B y_0$, 并整理, 得
$$
A\left(x_1-x_0\right)+B\left(y_1-y_0\right)=-\left(A x_0+B y_0+C\right) \text {. }
$$
将(4)与(6)两边平方后相加可得
$$
